...
public class SimulAnnealEpsDynamic extends Search {
	...
	public SimulAnnealEpsDynamic(Objective fn, Set<Objective> boundedFns,
	...
	public Set<Candidate> calcOptimalSolns(Constraint constraint,
		...
		for (Objective bounded : boundedFns) {
			bounded.setPrevBestOut(bounded.out(currentSoln, tgtDataset,
					mDataset, maxPvt, maxInd, recSize));

			logger.log(ProdLevel.PROD,
					"Initial fn outs:" + bounded.getPrevBestOut());
		}

		// We should add the current soln because the initial thresholds are
		// based on the current soln.
		solns.add(currentSoln);
		...
		while (temperature > finalTemperature) {
			...
			for (int step = 0; step < Config.STEPS_PER_TEMP; step++) {
				...
				Map<Objective, Double> objToOut = new HashMap<>();
				...
				for (Objective bounded : boundedFns) {
					...
					satisfiesBound = satisfiesBound
							&& bOut <= bounded.getEpsilon()
									* bounded.getPrevBestOut();
					objToOut.put(bounded, bOut);
				}
				...
				if ((Math.abs(delta) <= Config.FLOAT_EQUALIY_EPSILON && satisfiesBound)
					...
					for (Map.Entry<Objective, Double> entry : objToOut
							.entrySet()) {
						entry.getKey().setPrevBestOut(entry.getValue());
					}

					// logger.log(DebugLevel.DEBUG,
					// "New fn outs which satisfy threshold:" + objToOut);

					if (Math.abs(delta) <= Config.FLOAT_EQUALIY_EPSILON
							&& satisfiesBound) {
						logger.log(DebugLevel.DEBUG, "Neighb is equally good.");
					} else {
				...
				if (Math.abs(newFnOut - bestFnOut) <= Config.FLOAT_EQUALIY_EPSILON
					...
					// logger.log(DebugLevel.DEBUG,
					// "Removing all solns and adding best one.");
				}
